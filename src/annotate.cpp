...
std::string build_command_line(const AnnotateOptions& options) {
  ...
  out << "pgbam annotate-bam --bam " << options.bam_path
...
std::string make_gaf_qname_order_error(const std::string& path,
  ...
  out << "GAF must be sorted by qname in column 1: " << path
      ...
      << "'; use: LC_ALL=C sort -k1,1V -k12,12nr input.gaf > sorted.gaf)";
  return out.str();
}
    ...
    workers.emplace_back([&]() {
      try {
        while (true) {
          ...
          result.subpaths = collect_subpaths(graph, item.walks);
    ...
    try {
      ...
      while (true) {
        ...
        while (true) {
          ...
          const ReadAnnotation annotation = build_annotation(item.subpaths, interner);
