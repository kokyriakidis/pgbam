...
struct SortGafOptions {
  std::string in_path;
  std::string out_path;
};

struct CommandLine {
  enum class Command { AnnotateBam, Decode, SortGaf };

  Command command = Command::AnnotateBam;
  AnnotateOptions annotate_bam;
  DecodeOptions decode;
  SortGafOptions sort_gaf;
};
