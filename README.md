...
pgbam bridges pangenome graph alignments and the BAM ecosystem. Given a set of reads aligned to a pangenome graph (GAF format) and a haplotype index (GBWT or GBZ), it annotates a BAM with haplotype-path information and writes a compact sidecar file that maps those annotations back to named samples and haplotypes.

The typical workflow is:
1. Align reads to a pangenome graph (e.g. with `vg giraffe`) → produces a GAF file.
2. Run `pgbam annotate-bam` to produce an annotated BAM and a `.pgbam` sidecar.
...
- [Usage](#usage)
  - [`pgbam annotate-bam`](#pgbam-annotate-bam)
  - [`pgbam sort-gaf`](#pgbam-sort-gaf)
...
pgbam sort-gaf \
    --in  HG002.gaf \
    --out HG002.qname.mapq.gaf
# or equivalently with GNU sort:
# LC_ALL=C sort -k1,1V -k12,12nr HG002.gaf > HG002.qname.mapq.gaf
```

**Step 3 — Annotate the BAM:**
...
`pgbam annotate-bam` outputs reads in qname order. To query by genomic region, coordinate-sort and index the annotated BAM:

```bash
samtools sort \
    -@ 8 \
    -o HG002.annotated.coord.bam \
    HG002.annotated.bam
...
`pgbam annotate` is kept as a compatibility alias for `pgbam annotate-bam`.
...
- The GBZ or GBWT index is loaded into RAM before annotation starts; it is not streamed from disk during annotation.
- If an r-index is provided, it is also loaded into RAM rather than streamed.
- In practice, RAM usage is therefore driven primarily by the graph index files, not by the BAM/GAF join itself.

**Input requirements:**

- `pgbam annotate-bam` requires both BAM and GAF to be non-decreasing by `qname`.
- Sort BAM by query name with `samtools sort -n -o reads.qname.bam reads.bam`.
- Sort GAF by `qname` and then decreasing MAPQ with `pgbam sort-gaf --in reads.gaf --out reads.qname.mapq.gaf` (or `LC_ALL=C sort -k1,1V -k12,12nr reads.gaf > reads.qname.mapq.gaf`).
